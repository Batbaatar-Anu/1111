...
if(isset($_POST['but_submit'])){
    ...
    if($uname != " " && $password != " "){
        $sql_query="SELECT count(*) AS u_id FROM users WHERE username='".$uname."' AND password='".$password."'";
        $result = mysqli_query($con,$sql_query);
        $row= mysqli_fetch_array($result);
        $count = $row['u_id'];

        $pmission="SELECT u_id, permission FROM users WHERE username='".$uname."' AND password='".$password."'";
